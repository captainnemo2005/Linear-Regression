...
def compute_gradient(current_b, current_m, points, learning_rate,num_interation):
    ...
    for i in range (num_interation):
        b,m = step_gradient(current_b,current_m,points,learning_rate)
    return [b, m]
...
def run():
    ...
    print("Calculating ......")
    [b,m] = compute_gradient(initial_b,initial_m,array(points),learning_rate,num_interation)
    print("With gradient descent Algorithm, b = {1}, m = {1}, error = {2}".format(b,m,computing_error_for_certain_point(b, m,points)))
